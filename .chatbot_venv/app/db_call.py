...
def preprocess(message):
    ...
    tokens = [token for token in tokens if token not in stopwords.words('ru')]
    lemmatized_tokens = [lemmatizer.lemmatize(token) for token in tokens]
    stemmed_tokens = [stemmer.stem(token) for token in lemmatized_tokens]
    return ' '.join(stemmed_tokens)

def preprocess_with_stopwords(message):
    lemmatizer = WordNetLemmatizer()
    stemmer = PorterStemmer()
    text = re.sub(r'[^\w\s]', '', message)
    tokens = nltk.word_tokenize(text.lower())
...
class QASystem:
    ...
    def get_response(self, message):
        processed_text = preprocess_with_stopwords(message)
        ...
        if max_similarity > 0.4:
            ...
            processed_text_with_stopwords = preprocess_with_stopwords(message)
